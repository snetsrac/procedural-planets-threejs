/**
 * Simplex noise algorithm adapted from the paper "Simplex noise demystified" by
 * Stefan Gustavson: https://weber.itn.liu.se/~stegu/simplexnoise/simplexnoise.pdf
 */

import * as THREE from "three";

interface NoiseSettings {
  roughness: number;
  strength: number;
  center: THREE.Vector3;
}

interface NoiseGenerator {
  evaluate: (point: THREE.Vector3) => number;
}

const DEFAULT_NOISE_SETTINGS: NoiseSettings = {
  roughness: 1,
  strength: 1,
  center: new THREE.Vector3(0, 0, 0)
};

const NOISE_SETTINGS_LIMITS = {
  minRoughness: 0,
  maxRoughness: 5,
  minStrength: 0,
  maxStrength: 1,
  minCenter: -2,
  maxCenter: 2
};

// Simplex noise in 3D
function createNoiseGenerator(seed?: number): NoiseGenerator {
  // Gradient vectors (midpoint of each edge of a unit cube)
  const grad3 = [
    new THREE.Vector3(1, 1, 0), new THREE.Vector3(-1, 1, 0), new THREE.Vector3(1, -1, 0), new THREE.Vector3(-1, -1, 0),
    new THREE.Vector3(1, 0, 1), new THREE.Vector3(-1, 0, 1), new THREE.Vector3(1, 0, -1), new THREE.Vector3(-1, 0, 1),
    new THREE.Vector3(0, 1, 1), new THREE.Vector3(0, -1, 1), new THREE.Vector3(0, 1, -1), new THREE.Vector3(0, -1, -1)
  ];

  // Permutation table (originally from Ken Perlin)
  const p = [
    151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142,
    8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203,
    117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171, 168, 68, 175, 74,
    165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133, 230, 220,
    105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54, 65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132,
    187, 208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3,
    64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227,
    47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221,
    153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185,
    112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241, 81, 51,
    145, 235, 249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121,
    50, 45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78,
    66, 215, 61, 156, 180
  ];

  // To remove the need for index wrapping, double the permutation table length
  const perm: number[] = [];

  for (let i = 0; i < 512; i++) {
    perm[i] = p[i & 255];
  }

  // Skewing and unskewing factors for 3 dimensions
  const F3 = 1 / 3;
  const G3 = 1 / 6;

  // 3D simplex noise
  function evaluate(point: THREE.Vector3): number {
    // Noise contributions from the four corners
    let n0, n1, n2, n3;

    // Skew the input space to determine which simplex cell we're in.
    // Very nice and simple skew factor for 3D
    const s = (point.x + point.y + point.z) * F3;
    const i = Math.floor(point.x + s);
    const j = Math.floor(point.y + s);
    const k = Math.floor(point.z + s);
    const t = (i + j + k) * G3;

    // Unskew the cell origin back to (x, y ,z) space.
    // The x, y, z distances from the cell origin.
    const x0 = point.x - i + t;
    const y0 = point.y - j + t;
    const z0 = point.z - k + t;

    // For the 3D case, the simplex shape is a slightly irregular tetrahedron.
    // Determine which simplex we are in.
    // Offsets for the second corner of simplex in (i, j, k) coords
    let i1, j1, k1;
    // Offsets for the third corner of simplex in (i, j, k) coords
    let i2, j2, k2;

    if (x0 >= y0) {
      if (y0 >= z0) {
        // XYZ order
        i1 = 1;
        j1 = 0;
        k1 = 0;
        i2 = 1;
        j2 = 1;
        k2 = 0;
      } else if (x0 >= z0) {
        // XZY order
        i1 = 1;
        j1 = 0;
        k1 = 0;
        i2 = 1;
        j2 = 0;
        k2 = 1;
      } else {
        // ZXY order
        i1 = 0;
        j1 = 0;
        k1 = 1;
        i2 = 1;
        j2 = 0;
        k2 = 1;
      }
    } else {
      if (y0 < z0) {
        // ZYX order
        i1 = 0;
        j1 = 0;
        k1 = 1;
        i2 = 0;
        j2 = 1;
        k2 = 1;
      } else if (x0 < z0) {
        // YZX order
        i1 = 0;
        j1 = 1;
        k1 = 0;
        i2 = 0;
        j2 = 1;
        k2 = 1;
      } else {
        // YXZ order
        i1 = 0;
        j1 = 1;
        k1 = 0;
        i2 = 1;
        j2 = 1;
        k2 = 0;
      }
    }

    // A step of (1, 0, 0) in (i, j, k) means a step of (1 - c, -c, -c) in (x, y, z),
    // a step of (0, 1, 0) in (i, j, k) means a step of (-c, 1 - c, -c) in (x, y, z),
    // a step of (0, 0, 1) in (i, j, k) means a step of (-c, -c, 1 - c) in (x, y, z),
    // where c = 1/6.
    // Offsets for the second corner in (x, y, z) coords (G3 == 1 / 6)
    const x1 = x0 - i1 + G3;
    const y1 = y0 - j1 + G3;
    const z1 = z0 - k1 + G3;
    // Offsets for the third corner in (x, y, z) coords. (2 * G3 == F3 == 1 / 3)
    const x2 = x0 - i2 + F3;
    const y2 = y0 - j2 + F3;
    const z2 = z0 - k2 + F3;
    // Offsets for the last corner in (x, y, z) coords. (3 * G3 == 0.5)
    const x3 = x0 - 1 + 0.5;
    const y3 = y0 - 1 + 0.5;
    const z3 = z0 - 1 + 0.5;

    // Work out the hashed gradient indices of the four simplex corners
    const ii = i & 255;
    const jj = j & 255;
    const kk = k & 255;
    const gi0 = perm[ii + perm[jj + perm[kk]]] % 12;
    const gi1 = perm[ii + i1 + perm[jj + j1 + perm[kk + k1]]] % 12;
    const gi2 = perm[ii + i2 + perm[jj + j2 + perm[kk + k2]]] % 12;
    const gi3 = perm[ii + 1 + perm[jj + 1 + perm[kk + 1]]] % 12;

    // Calculate the contribution from the four corners
    let t0 = 0.5 - (x0 * x0) - (y0 * y0) - (z0 * z0);
    if (t0 < 0) {
      n0 = 0;
    } else {
      t0 *= t0;
      n0 = t0 * t0 * grad3[gi0].clone().dot(new THREE.Vector3(x0, y0, z0));
    }

    
    let t1 = 0.5 - (x1 * x1) - (y1 * y1) - (z1 * z1);
    if (t1 < 0) {
      n1 = 0;
    } else {
      t1 *= t1;
      n1 = t1 * t1 * grad3[gi1].clone().dot(new THREE.Vector3(x1, y1, z1));
    }

    
    let t2 = 0.5 - (x2 * x2) - (y2 * y2) - (z2 * z2);
    if (t2 < 0) {
      n2 = 0;
    } else {
      t2 *= t2;
      n2 = t2 * t2 * grad3[gi2].clone().dot(new THREE.Vector3(x2, y2, z2));
    }

    
    let t3 = 0.5 - (x3 * x3) - (y3 * y3) - (z3 * z3);
    if (t3 < 0) {
      n3 = 0;
    } else {
      t3 *= t3;
      n3 = t3 * t3 * grad3[gi3].clone().dot(new THREE.Vector3(x3, y3, z3));
    }

    // Add contributions from each corner to get the final noise value.
    // The result is scaled to stay just inside [-1, 1]
    return (n0 + n1 + n2 + n3) * 32;
  }

  return { evaluate };
}

export { NoiseSettings, DEFAULT_NOISE_SETTINGS, NOISE_SETTINGS_LIMITS, createNoiseGenerator };
